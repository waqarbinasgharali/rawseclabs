...
export default function ServiceMatrix() {
  ...
  return (
    <section id="categories" className="py-24 bg-[#050811] relative overflow-hidden border-t border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            ...
            <p className="mt-4 text-base text-slate-400 max-w-2xl leading-relaxed">
              From CREST-accredited penetration testing and red teaming to smart contract formal verification and ISO 27001 readiness, RawSecLabs delivers end-to-end cyber resilience.
            </p>
          </div>

          <Link
            href="/services"
            className="self-start md:self-auto px-6 py-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] text-slate-200 border border-white/10 hover:border-cyan-500/40 text-sm font-semibold transition-all flex items-center gap-2 group whitespace-nowrap"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredCategories.map((cat) => (
            <div
              key={cat.id}
              className={`group flex flex-col justify-between p-6 rounded-2xl bg-[#090e1b]/80 border border-white/10 backdrop-blur-xl shadow-xl transition-all duration-300 ${borderHoverMap[cat.slug] || 'hover:border-cyan-500/50'}`}
            >
              <div>
                {/* Card Header */}
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="px-2.5 py-1 rounded-full text-[11px] font-mono font-bold bg-white/[0.04] text-slate-400 border border-white/10 group-hover:text-cyan-400 transition-colors">
                ...
                <div className="mt-5 pt-4 border-t border-white/5 space-y-2">
                  <p className="text-[10px] uppercase tracking-wider font-mono text-slate-500 font-semibold">Featured Engagements:</p>
                  {cat.services.slice(0, 3).map((sub) => (
                    <Link
                      key={sub.slug}
                      href={`/services/${cat.slug}/${sub.slug}`}
                      className="text-xs text-slate-300 hover:text-cyan-300 flex items-center justify-between py-1 group/item transition-colors"
                    >
              ...
              <div className="mt-6 pt-4 border-t border-white/5">
                <Link
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-white/[0.02] hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/30 text-xs font-semibold text-slate-300 hover:text-cyan-300 transition-all flex items-center justify-center gap-2 group/btn"
                >
